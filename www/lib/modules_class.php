...
abstract class Modules {
	...
	protected function getBlogArticles($articles, $page) {
		...
		for ($i = $start; $i < $end; $i++) {
			...
			$sr["img_link"] = $this->config->dir_img."articles/".$articles[$i]["img_link"];
			$sr["width"] = $articles[$i]["width"];
			$sr["height"] = $articles[$i]["height"];
	...
	protected function getPagination($count, $count_on_page, $link) {
		...
		//if ((($this->data["page"] > 0) && ($this->data["page"] <= $count_pages)) || ($link === $this->config->address)) {
			$sr["number"] = 1;
			$sr["link"] = $link;
